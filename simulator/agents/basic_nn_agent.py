"""Implementation of a simple NeuralNetworkAgent that can compete in 1v1s with the three starters."""
...
class BasicNeuralNetworkAgent(NeuralNetworkAgent, metaclass=ABCMeta):
    """A basic NeuralNetworkAgent that bases its decisions solely on the status of the current ActivePokemon.

    Its Pokedex is limited to the three starters.
    """

    @staticmethod
    def __encode_one_hot_pokedex(pokemon: PokemonSpecies) -> np.ndarray:
        """Encodes a one-hot representation of a Pokemon species as a column vector.

        Args:
            pokemon (PokemonSpecies): The PokemonSpecies to encode.

        Returns:
            np.ndarray: A vector representation of the given PokemonSpecies.

        Raises:
            ValueError: The PokemonSpecies must be Bulbasaur, Charmander, or Squirtle.
        """
        encoding = np.zeros((3, 1))
        if pokemon.dex_num in (1, 4, 7):
            index = pokemon.dex_num // 3
        else:
            raise ValueError("The Basic Ruleset only allows for the three starter Pokemon.")
        encoding[index, 0] = 1
        return encoding

    @staticmethod
    def __encode_stat_modifiers(stat_modifiers: List[int]) -> np.ndarray:
        """Encodes the given list of status modifiers as a column vector.

        Args:
            stat_modifiers (List[int]): A six-element-long list of stat modifiers.

        Returns:
            np.ndarray: A vector representation of the given stat modifiers.
        """
    ...
    def __encode_one_hot_status(status: Status) -> np.ndarray:
        """Encodes a one-hot representation of a Status as a column vector.

        Args:
            status (Status): The Status to encode.

        Returns:
            np.ndarray: A vector representation of the given Status.
        """
    ...
    def __encode_active_pokemon(self, pokemon: ActivePokemon) -> np.ndarray:
        """Encodes a representation of an ActivePokemon as a column vector.

        Args:
            pokemon (ActivePokemon): The ActivePokemon to encode.

        Returns:
            np.ndarray: A vector representation of the given ActivePokemon.
        """
        return np.vstack((
